 ...
 * @example Todo
 *
 */class T extends t.TAbstractDataInserter{constructor(e={}){super(e),this.mergeStrategy="add"}static _arrayify(e){let t=[];return i.isDefined(e)&&(t=i.isArray(e)?e:[e]),t}async _save(e,t,r,o,n){const a=T._arrayify(e);if(i.isEmptyArray(a))return void n("No data to save in database. Abort insert !");const c=a.map(e=>e.name);console.log(`ThreeToMongoDB: Saving ${c}`),t.mergeStrategy&&(this.mergeStrategy=t.mergeStrategy);try{const e=t.parentId;let o=null,c=null;if(i.isDefined(e)){const r=this._readOneDocument("Objects3D",{_id:e});if(i.isNull(r))return void n(`Unable to retrieve parent with id (${t.parentId}). Abort insert !`);if("add"===this.mergeStrategy)o=await this._parseObjects(a,e),c=i.isDefined(o)?o.filter(e=>e).map(e=>e._id):[],this._updateDocument(r,{$addToSet:{children:c}});else if("replace"===this.mergeStrategy){const t=this._readManyDocument("Objects3D",{parent:e}),n=a.map(e=>e.uuid),s=t.filter(e=>!n.includes(e.uuid));this._removeChildrenDocuments(s),o=await this._parseObjects(a,e),c=i.isDefined(o)?o.filter(e=>e).map(e=>e._id):[],this._updateDocument(r,{$set:{children:c}})}}else o=await this._parseObjects(a,null),c=i.isDefined(o)?o.filter(e=>e).map(e=>e._id):[];console.log(`ThreeToMongoDB: Saved ${c}`),r()}catch(e){n(e)}}async _parseObjects(e=[],t=null){const r=T._arrayify(e);if(i.isEmptyArray(r))return null;const o=[];let n=null;for(let e=0,i=r.length;e<i;e++)n=this._parseObject(r[e],t),o.push(n);return Promise.all(o)}async _parseObject(e,t=null){if(i.isNotDefined(e))return null;const r=e.type,o=e.name,n=e.geometry,a=T._arrayify(e.children),c=T._arrayify(e.material);if(i.isDefined(n)&&i.isEmptyArray(a))if(n.isGeometry){const e=n.vertices;if(i.isNotDefined(e)||i.isEmptyArray(e))return console.error(`Leaf object ${o} have a geometry that doesn't contain vertices ! Skip it.`),null}else{if(!n.isBufferGeometry)return console.error(`Object ${o} contain an unknown/unmanaged geometry of type ${n.type} ! Skip it.`),null;{const e=n.attributes;if(i.isNotDefined(e))return console.error(`Buffer geometry of ${o} doesn't contain attributes ! Skip it.`),null;const t=e.position;if(i.isNotDefined(t)||0===t.count)return console.error(`Leaf object ${o} have a buffer geometry that doesn't contain position attribute ! Skip it.`),null}}let s=null;if(T.AvailableLineTypes.includes(r)){if(i.isNotDefined(n))return console.error(`Missing geometry for object ${e.name} of type ${r}. Only Sprite can contains material without geometry ! Skip it.`),null;s=T.AvailableLineMaterialTypes}else if(T.AvailablePointTypes.includes(r)){if(i.isNotDefined(n))return console.error(`Missing geometry for object ${e.name} of type ${r}. Only Sprite can contains material without geometry ! Skip it.`),null;s=T.AvailablePointMaterialTypes}else T.AvailableSpriteTypes.includes(r)&&(s=T.AvailableSpriteMaterialTypes);if(s)for(let e=0,t=c.length;e<t;e++){const t=c[e].type;if(!s.includes(t))return console.error(`Object ${o} of type ${r}, contain an invalid material of type ${t} ! Skip it.`),null}const d=this._getOrCreateDocuments(n),u=i.isDefined(d)?d.filter(e=>e).map(e=>e._id).pop():null,m=this._getOrCreateDocuments(c),l=i.isDefined(m)?m.filter(e=>e).map(e=>e._id):[];let f=this._readOneDocument(r,{uuid:e.uuid,parent:t});if(i.isDefined(f))if("add"===this.mergeStrategy){const e=this._parseObjects(a,f._id),t=i.isDefined(e)?e.filter(e=>e).map(e=>e._id):[];this._updateDocument(f,{$addToSet:{children:t}})}else if("replace"===this.mergeStrategy){const e=this._readManyDocument("Objects3D",{parent:f._id}),t=a.map(e=>e.uuid),r=e.filter(e=>!t.includes(e.uuid));this._removeChildrenDocuments(r);const o=this._parseObjects(a,f._id),n=i.isDefined(o)?o.filter(e=>e).map(e=>e._id):[];this._updateDocument(f,{$set:{children:n,geometry:u,material:l}})}else console.error(`Unknown/Unmanaged merge srategy ${this.mergeStrategy}`);else{e.parent=t,e.children=[],e.geometry=u,e.material=l,f=this._createDocument(e);const r=this._parseObjects(a,f._id),o=i.isDefined(r)?r.filter(e=>e).map(e=>e._id):[];this._updateDocument(f,{$set:{children:o,geometry:u,material:l}})}return f}_getOrCreateDocuments(e=[]){const t=T._arrayify(e);if(i.isEmptyArray(t))return null;const r=[];for(let e=0,o=t.length;e<o;e++){const o=this._getOrCreateDocument(t[e]);r.push(o)}return Promise.all(r)}_getOrCreateDocument(e){if(i.isNotDefined(e))return null;let t=this._readOneDocument(e.type,{uuid:e.uuid});return t=i.isDefined(t)?this._updateDocument(t,e):this._createDocument(e),t}_createDocuments(e=[]){const t=T._arrayify(e);if(i.isEmptyArray(t))return null;const r=[];for(let e=0,o=t.length;e<o;e++){const o=this._createDocument(t[e]);r.push(o)}return r}async _createDocument(e){if(i.isNotDefined(e))return null;const t=this._driver.model(e.type);return(await t(e).save())._doc}async _readOneDocument(e,t){if(i.isNotDefined(e)||i.isNotDefined(t))return null;const r=await this._driver.model(e).findOne(t).exec();return i.isDefined(r)?r._doc:null}async _readManyDocument(e,t){return i.isNotDefined(e)||i.isNotDefined(t)?null:await this._driver.model(e).find(t).exec().map(e=>e._doc)}_updateDocuments(e=[],t,r){const o=T._arrayify(e);if(i.isEmptyArray(o))return null;const n=[];for(let e=0,i=o.length;e<i;e++){const i=this._updateDocument(o[e],t,r);n.push(i)}return n}async _updateDocument(e,t,r){return i.isNotDefined(e)?null:await this._driver.model(e.type).findByIdAndUpdate(e._id,t,r).exec()}_deleteDocuments(e=[]){const t=T._arrayify(e);if(i.isEmptyArray(t))return null;const r=[];for(let e=0,o=t.length;e<o;e++){const o=this._deleteDocument(t[e]);r.push(o)}return r}async _deleteDocument(e){return i.isNotDefined(e)?null:await this._driver.model(e.type).findByIdAndDelete(e._id).exec()}_removeChildrenDocuments(e){for(let t=e.length-1;t>=0;t--)this._removeChildDocument(e[t])}_removeChildDocument(e){const t=this._readManyDocument("Objects3D",{parent:e._id});this._removeChildrenDocuments(t),this._removeOrphanGeometryWithId(e.geometry),this._removeOrphanMaterialsWithIds(e.material||[]),this._deleteDocument(e)}_removeOrphanGeometryWithId(e){if(i.isNotDefined(e))return;if(this._readManyDocument("Objects3D",{geometry:e}).length>1)return;const t=this._readOneDocument("Geometries",{_id:e});this._deleteDocument(t)}_removeOrphanMaterialsWithIds(e){const t=[];for(let r=0,o=e.length;r<o;r++){const o=e[r],n=this._removeOrphanMaterialWithId(o);t.push(n)}return t}_removeOrphanMaterialWithId(e){if(this._readManyDocument("Objects3D",{material:e}).length>1)return;const t=this._readOneDocument("Materials",{_id:e});this._deleteDocument(t)}}
/**
 * @author [Tristan Valcke]{@link https://github.com/Itee}
 * @license [BSD-3-Clause]{@link https://opensource.org/licenses/BSD-3-Clause}
 *
